...
class Manthan_Marketplace_Block_Adminhtml_Order_Edit extends Mage_Adminhtml_Block_Widget_Form_Container {
	 ...
	 public function __construct(){
		...
		$seller_id = Mage::getModel('marketplace/seller')->isSeller();
		$shipmentcollection = Mage::getModel('sales/order_shipment')->getCollection()
					->addFieldToFilter('order_id',$order_id)
					->addFieldToFilter('seller_id',$seller_id);
		if( $this->getRequest()->getControllerName() =="adminhtml_order" &&  $this->getRequest()->getActionName() == "view" && !$shipmentcollection->getSize())
		{
			$this->_addButton('ship', array(
				'label'     => Mage::helper('adminhtml')->__('Ship'),
				'onclick'   => 'setLocation(\'' . $this->getUrl('*/adminhtml_shipment/new',array('order_id'=>$this->getOrder()->getId())) . '\')',
				'class'     => 'scalable go',
			), 1);
		}
